...
기존에는 HuggingFace의 `eunsour/en-ko-transliterator` 모델을 사용했지만, 직접 파인튜닝한 모델로 대체하기로 했다. 리서치 결과 ByT5가 음차 변환에 적합하다고 판단했다.

#### ByT5 모델 선택 이유

ByT5는 Google Research에서 개발한 바이트 단위 Transformer 모델이다. 기존 T5와 달리 서브워드 토크나이저 없이 UTF-8 바이트를 직접 처리한다.

- **문자 레벨 처리**: 철자와 발음 패턴을 직접 학습 가능
- **OOV 문제 없음**: 새로운 고유명사나 신조어도 처리 가능
- **다국어 지원**: 100+ 언어로 사전학습되어 한글 바이트 패턴에 익숙

#### try #1 - 영어 → 한글 직접 학습 (실패)

처음에는 영어 단어를 바로 한글 음차로 변환하는 방식으로 학습을 시도했다.

~~~
학습 설정:
- 모델: google/byt5-base (580M params)
- 데이터: 37,530개 영한 음차 쌍
- 하이퍼파라미터: LR 3e-4, batch 16, epoch 10
~~~

**결과: Gradient Explosion 발생**

~~~
{'loss': 0.0, 'grad_norm': nan, 'learning_rate': 1.2e-06, 'epoch': 0.27}
{'eval_loss': nan, ...}
~~~

loss가 0.0으로 떨어지고 grad_norm이 nan이 되는 현상이 발생했다. 학습률이 너무 높아 기울기가 폭발한 것으로 판단했다.

#### try #2 - 하이퍼파라미터 조정 후 재학습

~~~
조정된 설정:
- LR: 3e-4 → 5e-5 (낮춤)
- Batch: 16 → 8 (줄임)
- Gradient Accumulation: 4 (실제 배치 32)
- FP16 비활성화 (안정성 우선)
- Gradient Clipping: max_norm 1.0
~~~

이번에는 학습이 정상적으로 진행되었다.

~~~
{'loss': 6.28, 'grad_norm': 211.41, 'epoch': 0.11}
{'loss': 3.98, 'grad_norm': 57.98, 'epoch': 0.21}
{'loss': 1.59, 'grad_norm': 8.19, 'epoch': 0.53}
~~~

#### 테스트 결과 분석

학습된 모델로 테스트한 결과, 일부 케이스에서 문제가 발견되었다.

| 단어 | 정답 | 출력 | 문제 유형 |
|------|------|------|-----------|
| schedule | 스케쥴 | 셰델 | 묵음 처리 실패 |
| psychology | 사이콜로지 | 시콜로지 | p 묵음 처리 실패 |
| champagne | 샴페인 | 샌파인 | 외래어 발음 |
| concerthall | 콘서트홀 | 콘서톨 | 합성어 처리 실패 |

**분석 결과:**
1. 영어의 불규칙 발음 규칙(묵음 등)을 모델이 완전히 학습하지 못함
2. 합성어 경계를 인식하지 못해 발음이 뭉개짐
3. 프랑스어/이탈리아어 차용어의 원어 발음 처리 어려움

#### 새로운 파이프라인 설계

문제 해결을 위해 G2P(Grapheme-to-Phoneme)를 활용한 2단계 파이프라인을 설계했다.

~~~
영어 단어 "concerthall"
    ↓
[wordninja] → ["concert", "hall"]  # 합성어 분리
    ↓
[g2p-en] → ["K AA1 N S ER0 T", "HH AO1 L"]  # 발음기호 변환
    ↓
[ByT5 모델] → ["콘서트", "홀"]  # ARPABET → 한글
    ↓
[합성] → "콘서트홀"
~~~

**핵심 아이디어:**
- g2p-en이 묵음 처리를 담당 (CMU 발음사전 기반)
- wordninja가 합성어 분리를 담당
- ByT5는 발음기호 → 한글 매핑만 학습 (역할 단순화)

#### 데이터셋 변환

기존 데이터셋을 ARPABET 형식으로 변환했다.
...
# 변환 예시
# 기존: {"psychology": "사이콜로지"}
# 변환: {"S AY1 K AA1 L AH0 JH IY0": "사이콜로지"}
~~~

37,530개 데이터를 모두 ARPABET 형식으로 변환 후 재학습을 진행했다.

#### 최종 결과

ARPABET 파이프라인 적용 후 묵음 처리와 합성어 문제는 개선되었다. 그러나 새로운 문제가 발견되었다.

| 단어 | 정답 | 출력 | 문제 |
|------|------|------|------|
| pizza | 피자 | 피츠사 | 이탈리아어 발음 |
| latte | 라떼 | 로테이 | 이탈리아어 발음 |
| laboratory | 레보토리 | 라브라토리 | 영미식 발음 차이 |

**원인 분석:**
- g2p-en은 "영어식 발음"만 제공
- 원본 데이터셋의 한글 음차는 "원어 발음" 기준인 경우가 많음
- 이 불일치로 모델이 혼란을 겪음

#### 향후 개선 방향

1. **데이터셋 정제**: 영어식 발음 기준으로 한글 음차 통일
2. **다른 G2P 사용**: epitran 등 다국어 지원 G2P로 원어 발음 반영
3. **하이브리드 접근**: 외래어는 별도 사전으로 처리

현재 구현된 파이프라인은 `inference_arpabet_pipeline.py`에서 `Eng2KorTransliteratorPipeline` 클래스로 사용할 수 있으며, `readutils.py`의 `read_engbymodel()` 함수에 통합되어 텍스트 정규화 과정에서 자동으로 호출된다.